...
/*
fetch('https://api.github.com/users/puribey')
  .then(function(r) {
    return r.json();
  })
  .then(function(j) {
    console.log(j);
    document.getElementById('username').innerText = j.name;
    document.getElementById('bio').innerText = j.bio;
    document.getElementById('avatar').innerHTML = '<img src="'+j.avatar_url+'" alt="hola"/>';
  })
*/
...
function getUser(name) {
  fetch('https://api.github.com/users/' + name)
    ...
    .then(function(j) {
      response = j; //asigna el json a la variable response 
      assignValues();
...
function getFollowers(url) {
  fetch(url)
    ...
    .then(function(f) {
      followers = f; //asigna otro json de urls a la variable followers
      listFollowers();
    });
}
